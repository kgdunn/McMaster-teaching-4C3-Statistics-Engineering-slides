...
\begin{frame}\frametitle{Example: 4N4 exam grades}
	\begin{center}
		\includegraphics[width=.84\textwidth]{\imagedir/univariate/qqplot-4N4-2012-grades.png}
	\end{center}
\end{frame}
...
\begin{frame}\frametitle{Method 2: No reference set available?}
	...
	\begin{enumerate}
		...
		\item	From CLT (assumes data in A and data in B are independent):  {\color{myOrange}{\small $\longleftarrow$ but, but ... we know that's not true!}}
			\begin{itemize}
...
\begin{frame}\frametitle{What happens if the samples are not independent? [600-level]}
	...
	What happens to the standard deviation of $\bar{x}$? (simulate many sequences; calculate $\bar{x}$ then stddev of these $\bar{x}$ values)
