% TODO: add the Confidence interval for the system on the slide "Method 2b: No reference set, internal σ"
...
\begin{frame}\frametitle{The high cost of variability in your raw materials}
	...
	\end{itemize}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{\imagedir/concepts/variation/feedback-control-variance-reduction.png}
	\end{center}
\end{frame}
...
\begin{frame}\frametitle{{\color{myOrange}{Side discussion:}} What happens if the samples are not independent? [600-level]}
...
\begin{frame}\frametitle{Back to method 2 for testing differences}
	...
	{\small
		 ...
		 \hspace{-0.5cm} \fbox{${\color{blue} (\bar{x}_B - \bar{x}_A) - c_n \sqrt{\sigma^2 \left(\tfrac{1}{n_A} + \tfrac{1}{n_B}\right)}  } < {\color{red} \mu_B - \mu_A} < {\color{blue} (\bar{x}_B - \bar{x}_A) + c_n \sqrt{\sigma^2 \left(\tfrac{1}{n_A} + \tfrac{1}{n_B} \right)} }$}
...
\begin{frame}\frametitle{Example: BOD}
	...
	% s^2_p = 62.53 = (10*(5.9)^2 + 10*(9.5)^2)/20
	%   s_p = 7.908
	% DOF = 20
	
	% Confidence interval: (xM,bar-xD,bar) = (22.6 - 16.4) 
	% Confidence interval: c_t with 20 DOF at 95\% level = qt(0.975, 20) = 2.085963
	% Confidence interval: stddev = sqrt(sp^2*(1/nM + 1/nD)) = sqrt(7.908^2 * (1/11+1/11)) = 3.37
	% CI lower bound =  (xM,bar-xD,bar) - c_t * stddev = (22.6 - 16.4) - 2.086*3.37 = -0.83
	% CI upper bound =  (xM,bar-xD,bar) + c_t * stddev = (22.6 - 16.4) + 2.086*3.37 = 13.2
\end{frame}
...
\begin{frame}\frametitle{Back to the BOD example}
	\begin{center}
		\includegraphics[height=.9\textheight]{\imagedir/univariate/BOD-comparison-plot-flipped.png}
	\end{center}
\end{frame}
