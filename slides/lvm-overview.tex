\begin{frame}\frametitle{Multivariate data analysis: What we will cover}
	\begin{itemize}
...
\begin{frame}\frametitle{Extracting value from data}
	...
	Engineers deal with {\color{purple}{large quantities}} of data from many {\color{purple}{different sources}}. We can do some some interesting {\color{myGreen}{\emph{and profitable}}} things with these data:
...
\begin{frame}\frametitle{LVM for troubleshooting: contribution plots}

	Ask the LV model: what has changed between the two clusters of points
	\begin{columns}[c]
		\column{0.6\textwidth}
			\begin{center}
				\includegraphics[width=\textwidth]{\imagedir/examples/monomer-recovery/Monomer-recovery-contributions.jpg} 
			\end{center}
			\vfill
