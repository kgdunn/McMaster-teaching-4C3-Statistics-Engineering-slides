...
% 2015: Put slides "EWMA derivation example" before the recursive version of EWMA. It makes more sense, since you've just covered MA, which is a function of the raw data.
% 2015: also put in a derivation for the UCL and LCL of the MA chart.
...
\begin{frame}\frametitle{Example}
	...
	Data from 20 such $\bar{x}_k$ calculations {\footnotesize(the 20 $\times$ 5 raw data are not shown)}:
...
\begin{frame}\frametitle{Adjusting the chart's performance}
	...
	\begin{itemize}
		\item	Ideal goal: $\alpha \rightarrow 0$ \textbf{and} $\beta \rightarrow 0$
...
\begin{frame}\frametitle{CUSUM (\emph{\textbf{cu}}mulative \emph{\textbf{sum}}) charts}
	...
	\end{block}
	\begin{itemize}
		\item	When a process shifts by $\Delta$, then we are adding $\Delta$ to every $x_t$
		\item	This accumulates: creates a steep up or down slope
	\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{CUSUM charts: ``out-of-control''}
	\begin{itemize}
		\item	Shift of 1.2 units $(1.2/3 = 0.4\sigma)$ starts at $t=150$, caught at around $t=180$; far earlier than a Shewhart chart
	\end{itemize}
	\begin{center}
...
\begin{frame}\frametitle{Using a CUSUM chart}
	...
	CUSUM charts:
	\begin{itemize}
		\item	should be used when we require very sensitive monitoring around a \textbf{specified, fixed target}
	\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{EWMA derivation example}
	...
	\begin{columns}[t]
		\column{1.1\textwidth}
			...
			\begin{align*}
				...
				\lambda = 0.8: && \hat{x}_{t+1} &=& 0.8 x_t     &+& 0.16 x_{t-1}               &+& 0.032 x_{t-2}                &+& 0.006 x_{t-3} &+ \ldots      \\							   
				\lambda = 0.6: && \hat{x}_{t+1} &=& 0.6 x_t     &+& 0.24 x_{t-1}               &+& 0.096 x_{t-2}                &+& 0.0384 x_{t-3} &+ \ldots      \\
				\lambda = 0.2: && \hat{x}_{t+1} &=& 0.2 x_t     &+& 0.16 x_{t-1}               &+& 0.128 x_{t-2}                &+& 0.1024 x_{t-3} &+ \ldots       \\ \hline
	...
	* MA chart with 4 samples per group: weight equals 1/4 = 0.25
	...
	\begin{itemize}
		...
		\item	As $\lambda \rightarrow 1$: chart uses more current data
	\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{EWMA derivation}
	...
	We would rather use a recursive formula though (so we don't have to store all the raw $x_t$ values in memory)

	\vspace{12pt}
	$
	\begin{array}{rcl}
		x_t &=& \text{raw data value at time step } t\\
		\hat{x}_{t+1} &=& \hat{x}_{t} + \lambda e_{t} \\
		e_t &=& x_t - \hat{x}_t \\
		\\
		\hat{x}_{t+1} &=& \hat{x}_{t} + \lambda e_{t} ={\color{myOrange} \hat{x}_{t}} + \lambda( {\color{myOrange} {x}_{t}} - {\color{myOrange} \hat{x}_{t}}) \\
		\\
	\end{array}
	$
	...
	\vspace{12pt}
	Only store $x_t$ and $\hat{x}_t$ to calculate $x_{t+1}$
	
\end{frame}

\begin{frame}\frametitle{EWMA derivation: an alternative interpretation}
	...
	\vspace{12pt}
	\begin{itemize}
		\item	As $\lambda \rightarrow 0$: smoother chart, uses more history, less current data (more CUSUM like)
...
\begin{frame}\frametitle{EWMA visual example (fault occurs at $t=150$)}
	\begin{center}
...
\begin{frame}\frametitle{EWMA limits}
	...
	\begin{itemize}
		...
		\item	the EWMA helpfully also gives a one-step ahead prediction
	\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{Example}
	...
	% Is is symmetric? No; it is closer to USL.
	% Cpk = (USL-x)/(3sigma) = (59 - 52.9)/(3 x 1.64) = 1.24
\end{frame}
